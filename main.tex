...
The key resource in continuous-variable teleportation is not a Bell pair of qubits, but a two-mode squeezed state. This state plays the role of an EPR pair. In the ideal EPR limit, the two modes have perfect correlations between their quadratures. 
In the convention used by Braunstein and Kimble\cite{PhysRevLett.80.869}, the ideal correlations are roughly
...
\end{equation}
This means that if Alice measures one part of the entangled pair, Bob's mode is strongly correlated with it. However, perfect EPR correlation would require infinite squeezing and therefore infinite energy. In reality that is impossible so the teleporatation is always approximated. 

The teleportation protocol proceeds as follows. Initially, Alice is given an unknown input state as Eq.\ref{Eq:Wigner_function_input},
which she wants to teleport to Bob. Alice and Bob also share a two-mode squeezed EPR state, such that Alice holds mode 1 and Bob holds mode 2.


First, Alice combines the unknown input mode with her half of the EPR pair on a 50/50 beam splitter Fig.\ref{fig:CV_teleportation_diagram}. The beam splitter produces two output modes,
...
\end{equation}
Using the EPR correlations in Eq.\ref{Eq:EPR_correlations}, we can see why this reconstructs the input state. For the $x$ quadrature,
\begin{equation}
    x_{\text{out}}
    =
    x_2 + \sqrt{2}x_a
