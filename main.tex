...
\[
    F = 0.80 \pm 0.01,
\]
which is clearly above the classical limit of \(2/3\).
...
For the result, they obtained an average gate fidelity of \(86.2(9)\%\) on QGT of a controlled-Z gate between two trapped-ion modules.
Moreover, the authors also showed that the same QGT process can be repeated several times to build more complicated two-qubit gates. For example, they implemented distributed iSWAP and SWAP gates. The iSWAP gate used two teleported CZ gates and reached a fidelity of \(70(2)\%\), while the SWAP gate used three teleported CZ gates and reached a fidelity of \(64(2)\%\).
...
a \in \{00,01,10,11\},
\]
the system found the correct answer with an average success probability of \(71(1)\%\).

\section{Conclusion}

In this report, we reviewed how quantum teleportation has developed from the original Bennett protocol\cite{Bennett} into several experimental platforms for quantum communication and quantum computation. 
Although the physical systems are different, the central idea is the same: an unknown quantum state is transferred using shared entanglement, a Bell-type measurement, classical communication, and a conditional correction. 
The main difference between platforms is their practical purpose. Optical photons are well suited for long-distance communication, continuous-variable optical modes allow unconditional teleportation of coherent states, atomic ensembles provide a possible quantum memory, solid-state circuits allow fast feed-forward control, and gate teleportation can connect separated quantum processors.
...
\bibliography{reference, title = {References}}
