...
\subsubsection{The Preparer: Encoding the Unknown State}
...
\begin{itemize}
    \item Polarization Initialization: Photon 1 enters with an initial defined polarization (typically vertical, $|v\rangle_1$).
    \item Unitary Manipulation: Quarter-wave plates and Fresnel rhomb rotators (R) transform this into a general pure state: $|\phi\rangle=\alpha|v\rangle_1+\beta|h\rangle_1$.
    \item Path Independence: Operating identically on paths $a_1$ and $b_1$, the polarization state effectively factors out of the path entanglement. 
\end{itemize}
...
\subsubsection{Alice's BSM Engine: Interferometric Bell State Measurement}
...
In contrast, when $r>0$, Alice and Bob share genuine EPR entanglement, and the teleportation fidelity($F = \langle \psi_{\text{in}} | \rho_{\text{out}} | \psi_{\text{in}} \rangle$) can exceed what's possible with classical methods (see Fig.\ref{fig:fidelity_plot}).
...
Before talking about teleporation using atomic ensembles, I think we quickly go over how we can prepare EPR via atoms first introduce by Hagley \textit{et al.} \cite{Hagley1997}.
In the experiment, two rubidium atoms passed one after another through a high-\(Q\) microwave cavity. The first atom was initially prepared in an excited circular Rydberg state \(\ket{e}\)\\($\mathrm{D_e}$ in Fig. \ref{fig:Hagley}), 
the second in a lower state \(\ket{g}\) ($\mathrm{D_g}$ in Fig. \ref{fig:Hagley}), and the cavity was initially in the vacuum state \(\ket{0}\).

\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\textwidth]{figures/EPR via atom setup.png}
    \caption{Diagram of the experiment by Hagley \textit{et al.} \cite{Hagley1997}.}
    \label{fig:Hagley}
\end{figure}

The initial state was therefore,
\begin{equation}
    \ket{\Psi_{\mathrm{in}}}
    =
    \ket{e_1,g_2,0}.
...
Thus, the cavity field acted as an intermediate quantum bus, which started and ended in the vacuum state, but during the process it mediated the entanglement between the two atoms. The atoms were separated by a distance on the order of a centimeter, showing that EPR-type entanglement could be produced between spatially separated massive particles.
The connection to atomic ensemble teleportation becomes clearer when one replaces individual atoms by collective atomic excitations. In an ensemble, the quantum information is not stored in one specific atom, but in a coherent collective mode shared among many atoms.
This collective enhancement increases the effective coupling between light and matter and allows the ensemble to behave like a single effective quantum memory. Depending on the encoding, atomic ensembles can be used either in a continuous-variable framework or in a discrete-variable spin-wave framework. In this subsection we will only discuss the continuous-variable approach, because it is relavant to the previous subsection. 

In the continuous-variable approach, the collective spin of an ensemble is treated as an effective harmonic oscillator. A representative experiment was performed by Sherson \textit{et al.}, in which a quantum state of light was teleported onto a macroscopic atomic ensemble containing approximately \(10^{12}\) cesium atoms \cite{Sherson2006}. The atoms were optically pumped into a coherent spin state with a large mean spin along the \(x\)-direction(see Fig.\ref{fig:Sherson}). The two transverse spin components could then be normalized as canonical variables,
\begin{equation}
    \hat{X}_A =
...
The performance of the teleportation is quantified by the fidelity between the input optical state and the reconstructed atomic state. Sherson \textit{et al.}\cite{Sherson2006} reported deterministic teleportation of coherent states with fidelities
$F = 0.60 \pm 0.02$ for $\bar{n}=5$, and  $F = 0.58 \pm 0.02$ for $\bar{n}=20$.

These values exceeded the corresponding classical benchmarks, showing that the process could not be explained as a purely classical measure-and-prepare transfer.
...
Optical-mode and atomic-ensemble systems fascinating as quantum teleporatation platforms, however,
they might be difficult to integrate into a large-scale quantum network or quantum computer.
Now we're going to discuss solid-state systems, where the quantum information is stored and processed in artificial quantum devices fabricated on a chip particuly superconduting circuits.
Instead of using freely propagating photons or naturally occurring atoms, this approach uses engineered macroscopic quantum systems whose quantum states behave like controllable two-level systems.

A representative experiment was demonstrated by Steffen \textit{et al.}\cite{Steffen2013}, who realized deterministic quantum teleportation with feed-forward in a superconducting circuit.
The system consists of three superconducting transmon qubits, denoted as $Q_1$, $Q_2$, and $Q_3$, 
coupled to three superconducting coplanar waveguide resonators(see Fig.\ref{fig:Steffen}). 

\begin{figure}[h]
    \centering
    \includegraphics[width=0.75\textwidth]{figures/solid state system setup.png}
    \caption{Diagram of the experiment by Steffen \textit{et al.} \cite{Steffen2013}, figure a is the connection diagram and the figure b-f is detail layout of the ICs.}
    \label{fig:Steffen}
\end{figure}
\newpage
Initially, $Q_1$ is the qubit carrying the unknown input state
\begin{equation}
...
\begin{equation}
    ...
    \right).
\end{equation}
This entangled pair plays the same role as the EPR pair in the original teleportation proposal. 
After this shared resource is created, the unknown state $|\psi_{\mathrm{in}}\rangle$ is prepared on $Q_1$.
The sender then performs a Bell-state measurement on $Q_1$ and $Q_2$. In the ideal circuit model, this Bell measurement can be understood as a transformation from the Bell basis to the computational basis, followed by ordinary qubit readout.
Experimentally, Steffen \textit{et al.} implemented the required two-qubit operations using controlled-PHASE(CPHASE) gates and single-qubit microwave rotations. The CPHASE gates are generated by tuning the transition frequencies of the transmon qubits with fast flux pulses, while the single-qubit rotations are driven by microwave pulses.
...
The feed-forward step applies this correction in real time. The measurement result from
$Q_1$ and $Q_2$ is sent to control electronics, which then applies the corresponding
microwave pulse to $Q_3$. In other words, feed-forward is the step that uses the sender's
measurement result to complete the teleportation on the receiver qubit.


A key achievement of this solid-state implementation is that it does not merely post-select one successful outcome. In many optical teleportation experiments, only part of the Bell basis can be distinguished, so the protocol succeeds only probabilistically. In this superconducting circuit experiment, Josephson parametric amplifiers were used to perform high-fidelity single-shot measurements. 
Moreover, the experiment implemented deterministic teleportation with active feed-forward, using all four possible Bell-measurement outcomes.

The reported results show that the teleported state fidelities are above the classical limit. For post-selected teleportation, the average output-state fidelity was about $0.817$, with an average process fidelity of about $0.720$. When all four Bell-state outcomes were distinguished deterministically, the average output-state fidelity was about $0.771$, with a process fidelity of about $0.655$. When real-time feed-forward was included, the directly measured deterministic state-transfer fidelity was about $0.688$, still above the classical threshold of $2/3$ for state teleportation. The lower fidelity in the full feed-forward case mainly came from imperfect qubit coherence, gate errors, readout errors, and the finite delay required for real-time measurement processing and conditional control.

Overall, the solid-state implementation of quantum teleportation provides alternative rather than photons or atoms with superconducting artificial atoms embedded in a integrated microwave circuit.
And with the integrated circuit platform, we can achieve deterministic teleportation with fast feed-forward(around 500$\mathrm{ns}$), which optical or atomic systems are hard to compare.
Still, for the three methods the overall protocol involves the same aspects, an unknown input state, a shared entangled pair, classical information communication, and a conditional correction.
...
In the previous sections, we detailed dicussed different physical platforms
for quantum teleporation. Based on the uses of those different platforms and method, 
the most advanced research focus on using quantum teleportation for quantum communication for potentially quantum internet or quantum computers.

A clear experimental example is the ground-to-satellite quantum teleportation
experiment performed using the Micius satellite(Fig. \ref{fig:satellite_teleportation})\cite{ren2017ground}. In this
...
\begin{figure}
    \centering
    \includegraphics[width=0.75\textwidth]{figures/Groud to satellite.png}
    \caption{Diagram of the ground-to-satellite quantum teleportation experiment by Ren \textit{et al.} \cite{ren2017ground}.}
    \label{fig:satellite_teleportation}
\end{figure}

Previous teleportation experiments using optical fibers or terrestrial free-space channels were limited
...
\]
The measured average teleportation fidelity was $F = 0.80 \pm 0.01$, which is clearly above the classical limit of \(2/3\).
...
While standard quantum teleportation transfers an unknown quantum state from one system to another, quantum gate teleportation (QGT) extends the same principle to quantum operations.
Instead of physically moving a qubit from one processor to another, QGT uses a previously shared entangled state, local operations, measurement, and classical feed-forward to implement a non-local gate between distant qubits.
This idea is especially important for quantum computing, because large-scale quantum processors require many qubits with high connectivity. Building a single device in which every qubit can directly interact with every other qubit is technically difficult.
QGT provides an opportunity to effectively create a large processor by connecting smaller modules together. 

A recent experimental demonstration by Main \textit{et al.}\cite{Main2025Distributed} shows how this idea can be used for distributed quantum computing across an optical network link.
In their architecture(see Fig. \ref{fig:QGT}(b)), two trapped-ion modules, labeled Alice and Bob, are separated by about two meters. Each module contains two functional types of qubits: a \textit{network qubit}, used to create remote entanglement through photons, and a \textit{circuit qubit}, 
used to store and process the computational quantum information. This separation of roles is essential. The network qubits interact with the optical link and generate entanglement, while the circuit qubits preserve the quantum information during the probabilistic entanglement-generation process.\\
\begin{figure}[h]
    \centering
    \includegraphics[width=0.75\textwidth]{figures/QGT.png}
    \caption{(a) conceptual diagram of the QGT with multiple modules, (b) QGT setup, (c) physical connection between multiple modules. By Main \textit{et al.} \cite{Main2025Distributed}.}
    \label{fig:QGT}
...
\right),
\]
which is generated by photon-mediated entanglement and heralded by detector clicks. 
Once this entanglement has been successfully produced, local controlled-Z operations are performed inside each module between the network qubit and the local circuit qubit(see Fig. \ref{fig:QGT_circuit}). 
The network qubits are then measured in chosen bases, and the two modules exchange the measurement outcomes through a classical communication channel. Finally, conditional single-qubit corrections are applied to the circuit qubits. The net effect is that the circuit qubits, which are physically located in different modules, undergo a non-local controlled-Z gate,
\[
...
\begin{figure}
    \centering
    \includegraphics[width=0.55\textwidth]{figures/QGT CZgate.png}
    \caption{Teleportation of a CZ gate between two trapped-ion modules. By Main \textit{et al.} \cite{Main2025Distributed}.}
    \label{fig:QGT_circuit}
\end{figure}
...
For the result, they obtained an average gate fidelity of $0.862(9)$ on QGT of a controlled-Z gate between two trapped-ion modules.
Moreover, the authors also showed that the same QGT process can be repeated several times to build more complicated two-qubit gates. For example, they implemented distributed iSWAP and SWAP gates. The iSWAP gate used two teleported CZ gates and reached a fidelity of $0.70(2)$, while the SWAP gate used three teleported CZ gates and reached a fidelity of $0.64(2)$.

Beyond individual gates, the experiment also ran a small distributed quantum algorithm: a two-qubit version of Grover's search algorithm. In this algorithm, one teleported CZ gate was used for the oracle, and another was used for the diffusion operation. For the four possible marked states,
\[
a \in \{00,01,10,11\},
\]
the system found the correct answer with an average success probability of $0.71(1)$.
\section{Conclusion}

In this report, we reviewed the development of quantum teleportation from its original theoretical idea\cite{Bennett} to several experimental realizations.
Quantum teleportation does not physically move a particle from one place to another. Yet, it reconstructs an unknown quantum state in a different system by using shared entanglement, a Bell-type measurement, classical communication, and a conditional correction. 
The examples discussed in this report show that this same basic protocol can be applied to many platforms, including optical qubits, continuous-variable optical modes, atomic ensembles, superconducting circuits, satellite-based communication, and quantum gate teleportation.
Each platform has a different advantage: photons are useful for long-distance communication, atomic ensembles are promising for quantum memories, solid-state systems allow fast feed-forward control, and gate teleportation provides a way to connect separated quantum processors. 
Although current experiments still face challenges such as loss, imperfect fidelity, limited efficiency, and scalability, they show that teleportation is no longer only a conceptual test of quantum mechanics. It has become an important tool for future quantum communication networks, quantum memories, and distributed quantum computation.
